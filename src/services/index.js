...
import { LUA_MASTER_FARM_ADDRESS, NUMBER_BLOCKS_PER_YEAR, TOKEN_ADDRESS } from '../constants/tokens'
  ...
  return route
    ...
    .then((res) => {
      if (_get(res, 'data.data')) {
        const convertedNumber = new BigNumber(res.data.data).div(10 ** 18).toNumber()
        if (typeof callback === 'function') {
...
const getPoolAPY = ({ pid, luaPrice, usdValue }, callback) => {
  ...
  return route
    ...
    .then((res) => {
      if (_get(res, 'data.data')) {
        const apy = (luaPrice * 10 ** 18 * NUMBER_BLOCKS_PER_YEAR * bnToDecimals(res.data.data)) / (usdValue * 100 ** 8)
...
const getTotalLiquidityData = (callback) => {
  ...
  const query = 'query transactions { uniswapFactories(first: 100) { id totalLiquidityUSD totalVolumeUSD } }'
  const variables = {}
  ...
  return graphRoute
    ...
    .then((res) => {
      const result = _get(res, 'data.data.uniswapFactories', []).map((item) => ({
